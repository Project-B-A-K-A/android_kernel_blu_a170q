...
#define __2IN1_DELAY_TIME__   2
#define __2IN1_GPIO_CONTROL_PIN__		18
#define RECEIVER_SPEAKER_2IN1_MODE_3 \
do { \
			msleep(2);\
			gpio_set_value(__2IN1_GPIO_CONTROL_PIN__, 0);\
			udelay(__2IN1_DELAY_TIME__);\
			gpio_set_value(__2IN1_GPIO_CONTROL_PIN__, 1);\
			udelay(__2IN1_DELAY_TIME__);\
			gpio_set_value(__2IN1_GPIO_CONTROL_PIN__, 0);\
			udelay(__2IN1_DELAY_TIME__);\
			gpio_set_value(__2IN1_GPIO_CONTROL_PIN__, 1);\
...
struct pinctrl *speakerctrl = NULL;
struct pinctrl_state *speaker_h = NULL;
struct pinctrl_state *speaker_l = NULL;


void speaker_gpio_init(struct platform_device *pdev)
{
	speakerctrl = devm_pinctrl_get(&pdev->dev);
		if (IS_ERR(speakerctrl)) 
			{
		pr_debug("Cannot find speakerctrl pinctrl!");
	}
	
	speaker_h = pinctrl_lookup_state(speakerctrl, "speaker_h");
	if (IS_ERR(speaker_h)) {
		pr_debug("Cannot find speaker_h pinctrl!");
	}
		speaker_l = pinctrl_lookup_state(speakerctrl, "speaker_l");
	if (IS_ERR(speaker_l)) {
		pr_debug("Cannot find speaker_l pinctrl!");
	}
	
	gpio_request(__2IN1_GPIO_CONTROL_PIN__,  NULL);
	gpio_direction_output(__2IN1_GPIO_CONTROL_PIN__, 1);
}
...
static void Ext_Speaker_Amp_Change(bool enable)
{
	if(enable)
	{
		printk("Ext_Speaker_Amp_Change ON+\n");
			//msleep(1000);
				RECEIVER_SPEAKER_2IN1_MODE_3;
		//pinctrl_select_state(speakerctrl, speaker_h);
			//MTK add for touch sound	
        	//msleep(30);
	...
	else
	{
		...
		//pinctrl_select_state(speakerctrl, speaker_l);
		gpio_set_value(__2IN1_GPIO_CONTROL_PIN__, 0);
	}
}
...
/*lipy add*/
#define RECEIVER_SPEAKER_2IN1_MODE_8 \
do { \
			msleep(2);\
			gpio_set_value(__2IN1_GPIO_CONTROL_PIN__, 0);\
			udelay(__2IN1_DELAY_TIME__);\
...
{		
		printk("Receiver_Speaker_Switch_Change %d+\n",enable);
		
		if(enable)
		{
			RECEIVER_SPEAKER_2IN1_MODE_8;
		}
		else
		{
			gpio_set_value(__2IN1_GPIO_CONTROL_PIN__, 0);
...
{
	pr_debug("%s()\n", __func__);
	printk("lipy ucontrol->value.integer.value[0] is %ld\n",ucontrol->value.integer.value[0]);
	printk("mCodec_data->mAudio_Ana_DevicePower[] is %d\n",mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_RECEIVER_SPEAKER_SWITCH]);
...
static const struct of_device_id speaker_gpio_control_ids[] = {
	{.compatible = "mediatek,speaker_gpio_control",},
	{}
};
#if 0
...
static struct platform_driver mtk_codec_6350_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
#ifdef CONFIG_OF
		   .of_match_table = speaker_gpio_control_ids,//mt_soc_codec_63xx_of_ids,
#endif
		   },
