...
static imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc = 32,//unit , ns //85
	...
	.normal_video = {
		...
		.grabwindow_width = 3264,
		.grabwindow_height = 2448,
		.mipi_data_lp2hs_settle_dc = 32,//unit , ns //85 4lane 32 2lane
		.max_framerate = 300,
	},
	.hs_video = {
		.pclk = 265600000,
	...
	.sensor_mode_num = 3,	  //support sensor mode num //10 4lane 3 2lane
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x0114,  0x01);//03 4lane //01 2lane
	...
	write_cmos_sensor(0x0172,  0x00);//add for 2lane
	...
	write_cmos_sensor(0x0307,  0x36);////2b 4lane //36 2lane
	write_cmos_sensor(0x0309,  0x0A);
	write_cmos_sensor(0x030B,  0x01);
	write_cmos_sensor(0x030C,  0x00);
	write_cmos_sensor(0x030D,  0x6c);//2e 4lane //6c 2lane
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 240) { //24fps for PIP
		//@@full_132PCLK_24.75
		  ...
		  write_cmos_sensor(0x0305,   0x03);
		  write_cmos_sensor(0x0306,   0x00);
		  write_cmos_sensor(0x0307,   0x36); //0x51 //53 4lane //36 2lane
		  write_cmos_sensor(0x0309,   0x0A);
		  write_cmos_sensor(0x030B,   0x01);
		  write_cmos_sensor(0x030C,   0x00);
		  write_cmos_sensor(0x030D,   0x6c);//0x54 //56 4lane //6c 2lane
		  write_cmos_sensor(0x455E,   0x00);
		  write_cmos_sensor(0x471E,   0x4B);
		  write_cmos_sensor(0x4767,   0x0F);
		  write_cmos_sensor(0x4750,   0x14);
