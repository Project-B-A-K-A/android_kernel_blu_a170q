...
#define LOG_1 LOG_INF("OV5670,MIPI 1LANE\n")
#define LOG_2 LOG_INF("preview 2592*1944@30fps,840Mbps/lane; video 2592*1944@30fps,840Mbps/lane; capture 5M@30fps,840Mbps/lane\n")
/****************************   Modify end    *******************************************/
#define LOG_INF(format, args...)	//xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)

/*****************************************************************************/
//	T CARD DEBUG
//	debug by sp_zhanghua 2015-06-03
/*****************************************************************************/
//#define DEBUG_SENSOR_OV5670		//for T-card
#ifdef DEBUG_SENSOR_OV5670	//sp_zhanghua
#include <asm/uaccess.h>
#include <linux/fs.h>
#include <linux/string.h>

#define OV5670_SD_FILE_PATH					"/mnt/sdcard/ov5670_sd"
...
static U32 OV5670_Initialize_from_T_Flash(char *file_path)
{
	U8 *curr_ptr = NULL;
	U8 *tmp = NULL;
	U32 file_size = 0;
	...
	OV5670_init_regs_struct OV5670_reg;
	...
	while (curr_ptr < (data_buff + file_size)) {
		...
		if (!strcmp((const char *)func_ind, "REG")) {		/* REG */
			curr_ptr += 4;				/* Skip "REG{" */
			OV5670_reg.addr = sp_strtoul((const char *)curr_ptr, &tmp, 16);
			curr_ptr = tmp;
			curr_ptr ++;	/* Skip "," */
			OV5670_reg.val = sp_strtoul((const char *)curr_ptr, &tmp, 16);
			curr_ptr += 2;	/* Skip "};" */
			
			//init
			SP_I2C_WRITE(OV5670_reg.addr, OV5670_reg.val);
			regs_total++;
		} else if (!strcmp((const char *)func_ind, "DLY")) {	/* DLY */
			curr_ptr += 4;				/* Skip "DLY(" */
			OV5670_reg.val = sp_strtoul((const char *)curr_ptr, &tmp, 10);
			curr_ptr = tmp;

			SP_DELAY(OV5670_reg.val);
		}
...
#define OV5670_OTHER_DEBUG_PATH					"/mnt/sdcard/OV5670_debug"
static imgsensor_info_struct imgsensor_info;
static void OV5670_other_debug(char *file_path)	//sp_zhanghua
{
	U8 *curr_ptr = NULL;
	U8 *tmp = NULL;
	U32 file_size = 0;
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.cap_delay_frame = 4,		//enter capture delay frame num//2
	.pre_delay_frame = 4, 		//enter preview delay frame num//2
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0303, 0x00);//gpw 
	...
	write_cmos_sensor(0x4800, 0x6c);     //4c   非连续点亮
...
#ifdef DEBUG_SENSOR_OV5670
	if(read_from_extern_flash(OV5670_SD_FILE_PATH)) {
		SP_LOG("________________from t!\n");
		OV5670_Initialize_from_T_Flash(OV5670_SD_FILE_PATH);
	} else {
		preview_setting();
		sensor_init();	
	}
...
#ifdef DEBUG_SENSOR_OV5670
	if(read_from_extern_flash(OV5670_OTHER_DEBUG_PATH)) {
		printk("sp_zhanghua: other debug!\n");
		OV5670_other_debug(OV5670_OTHER_DEBUG_PATH);
	}
#endif
