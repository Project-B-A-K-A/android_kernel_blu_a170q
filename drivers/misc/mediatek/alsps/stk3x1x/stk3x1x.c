...
#define DRIVER_VERSION          "3.5.2 20151201"
...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	0 //240
	#define STK_LT_N_CT	90  //120
	#define STK_HT_N_CT	95 //200 100
...
static int stk3x1x_hwmsen_write_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
...
int stk3x1x_master_send(struct i2c_client *client, u16 addr, u8 *buf ,int count)
{
	...
	while(retry++ < max_try)
	{
		ret = stk3x1x_hwmsen_write_block(client, addr, buf, count);
		if (ret == 0)
		    break;
		udelay(100);
	}
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	//	APS_LOG("%s: set HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low));		
		...
		if(true)
		{
			...
			if(obj->boot_cali == 1)
			{//	APS_LOG("%s: line=%d\n", __FUNCTION__, __LINE__);
				atomic_set(&obj->ps_threshold_high, obj->ps_high_thd_boot);
				atomic_set(&obj->ps_threshold_low, obj->ps_low_thd_boot);
			}
			else
			{	APS_LOG("%s: line=%d\n", __FUNCTION__, __LINE__);
			//	atomic_set(&obj->ps_threshold_high, 0xFFFF);
			//	atomic_set(&obj->ps_threshold_low, 0xFFFF);
			}
...
{    
	...
	if(buf[5] != (atomic_read(&obj->ps_threshold_high) & 0xFF00) >> 8)
	{
		APS_ERR("%s: invalid reg 0x06=0x%2x\n", __func__, buf[5]);
		return 0xFF;
	}	
	if(buf[6] != (atomic_read(&obj->ps_threshold_high) & 0x00FF))
	{
		APS_ERR("%s: invalid reg 0x07=0x%2x\n", __func__, buf[6]);
		return 0xFF;
	}	
	if(buf[7] != (atomic_read(&obj->ps_threshold_low) & 0xFF00) >> 8)
	{
		APS_ERR("%s: invalid reg 0x08=0x%2x\n", __func__, buf[7]);
		return 0xFF;
	}	
	if(buf[8] != (atomic_read(&obj->ps_threshold_low) & 0x00FF))
	{
...
#ifndef CALI_EVERY_TIME
	atomic_set(&obj->ps_threshold_high, obj->ps_stat_data[1] + obj->stk_ht_n_ct); 
	atomic_set(&obj->ps_threshold_low, obj->ps_stat_data[1] + obj->stk_lt_n_ct); 		
#else						
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
...
#endif
	{
		...
		if(ps_count > 50000){
			...
			if((ps_count%10) == 1){
				...
				word_data = (buf[0]<<8) | buf[1];	
				APS_LOG("%s: cur ps=%d, dis=%d\n", __func__, word_data, obj->ps_distance_last);							
			}			
		}

	 if(obj->psi_set != 0){
		if(obj->ps_distance_last != 0){
			ret = stk3x1x_master_recv(obj->client, 0x11, buf, 2);
			if(ret < 0)
			{
				APS_ERR( "%s fail, err=0x%x", __func__, ret);
			...
			if(ret == 0){
				if(word_data < 15){
					...
					ct = atomic_read(&obj->ps_threshold_high) - obj->stk_ht_n_ct;
					if((word_data < ct)&& ((ct - word_data)> 5) && (obj->ps_thd_update == false)){
						obj->psi = word_data;
						atomic_set(&obj->ps_threshold_high, word_data + obj->stk_ht_n_ct); 
						atomic_set(&obj->ps_threshold_low, word_data + obj->stk_lt_n_ct);
						if((err = stk3x1x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_threshold_high))))
						{
							APS_ERR("write high thd error: %d\n", ret);      
						}		
						...
						APS_LOG("tracking update HT=%d, LT=%d\n", atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low));											
					}
				}
			}		
		}
	...
	if(ret == 0)
	{
		...
		atomic_set(&obj->ps_threshold_high, obj->ps_high_thd_boot); 
		atomic_set(&obj->ps_threshold_low, obj->ps_low_thd_boot); 
		APS_LOG("%s: found boot HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low));
	...
	{
		obj->psi_set = obj->psi;
		atomic_set(&obj->ps_threshold_high, obj->psi + obj->stk_ht_n_ct); 
		atomic_set(&obj->ps_threshold_low, obj->psi + obj->stk_lt_n_ct); 
		
#ifdef CALI_EVERY_TIME
		if( (atomic_read(&obj->ps_threshold_high) > (obj->ps_high_thd_boot + 500)) && (obj->boot_cali == 1) )
		{
			atomic_set(&obj->ps_threshold_high, obj->ps_high_thd_boot); 
			atomic_set(&obj->ps_threshold_low, obj->ps_low_thd_boot); 
...
*/
		...
		if(sensor_data.values[0] == 1) //far
		{
			msleep(100);
			ret = stk3x1x_ps_val();
			if(ret == 0){
				APS_LOG("eint far update=%d\n", obj->ps_thd_update?1:0);						
				if(obj->ps_thd_update)
				{
					if((err = stk3x1x_read_ps(obj->client, &reading2)))
					{
						APS_ERR("stk3x1x read ps data: %d\n", err);
						goto err_i2c_rw;
					}			
					...
					if(obj->ps > reading2)
						value = obj->ps - reading2;
					else
						value = reading2 - obj->ps;
					APS_LOG("eint far r1 = %d, r2 = %d, ct = %d\n", obj->ps, reading2, ct);
					if(value < 30){
						if((obj->ps < reading2) && (obj->ps < (ct - 10))){
							atomic_set(&obj->ps_threshold_high, obj->ps + obj->stk_ht_n_ct); 
							atomic_set(&obj->ps_threshold_low, obj->ps + obj->stk_lt_n_ct); 
						}else if((reading2 < obj->ps) && (reading2 < (ct - 10))){
							atomic_set(&obj->ps_threshold_high, reading2 + obj->stk_ht_n_ct); 
							atomic_set(&obj->ps_threshold_low, reading2 + obj->stk_lt_n_ct); 
						}
						...
						APS_LOG("eint far update HT=%d, LT=%d\n", atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low));					
					}
					obj->ps_thd_update = false;
				}
			}
		...
		else
		{
			...
			APS_LOG("eint near r1=%d, r2=%d, psi=%d\n", obj->ps, reading2, obj->psi);	
			if(((reading2 - obj->psi) > 600) || ((obj->ps - obj->psi) > 600)){
				obj->ps_thd_update = true;
				atomic_set(&obj->ps_threshold_high, obj->psi + 160); 
				atomic_set(&obj->ps_threshold_low, obj->psi + 150); 
				if((err = stk3x1x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_threshold_high))))
				{
					APS_ERR("write high thd error: %d\n", ret);      
				}
				...
				APS_LOG("eint near update HT=%d, LT=%d\n", atomic_read(&obj->ps_threshold_high), atomic_read(&obj->ps_threshold_low));					
			}			
		}
#ifdef STK_PS_DEBUG
...
static int stk3x1x_get_ps_value(struct stk3x1x_priv *obj, u16 ps)
{
	...
	val = (flag & STK_FLG_NF_MASK)? 10 : 0;	
	if((ioctrl_ps_count%6) == 1){
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_IOCTL_CLR_CALI: 
			...
			atomic_set(&obj->ps_threshold_low,  obj->hw->ps_threshold_low);  //obj->hw->ps_threshold_low
			APS_LOG("ALSPS_IOCTL_CLR_CALI : obj->ps_cali:%d high:%d low:%d\n", obj->ps_cali, 
													atomic_read(&obj->ps_threshold_high), 
													atomic_read(&obj->ps_threshold_low)); 			
			break;
		...
		case ALSPS_IOCTL_SET_CALI:                 
			...
			atomic_set(&obj->ps_threshold_high, obj->ps_cali + obj->stk_ht_n_ct); 
			atomic_set(&obj->ps_threshold_low,  obj->ps_cali + obj->stk_lt_n_ct); 
			if((err = stk3x1x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_threshold_high))))
				goto err_out;       
			if((err = stk3x1x_write_ps_low_thd(obj->client, atomic_read(&obj->ps_threshold_low))))
				goto err_out;    
...
static int ps_get_data(int* value, int* status)
{
    ...
    else
    {
			...
			//obj->ps_distance_last = dat;
			if(stk3x1x_obj->ps > stk3x1x_obj->ps_max)
			{
				stk3x1x_obj->ps_max = stk3x1x_obj->ps;
			}
				
			if(stk3x1x_obj->ps < stk3x1x_obj->ps_min)
			{
				stk3x1x_obj->ps_min = stk3x1x_obj->ps;	
			}	

			ioctrl_ps_count ++;
			if(ioctrl_ps_count > 50000){
				ioctrl_ps_count = 0;	
			}else if((ioctrl_ps_count%6) == 1){
				APS_LOG("%s: update ps max=%d, min=%d, cur=%d\n", __func__, stk3x1x_obj->ps_max, stk3x1x_obj->ps_min, stk3x1x_obj->ps);		
			}			

			dat = stk3x1x_get_ps_value(stk3x1x_obj, stk3x1x_obj->ps);
			
			*value = dat;
			if(stk3x1x_obj->ps_distance_last != dat){
				stk3x1x_obj->ps_distance_last = dat;				
#ifdef STK_PS_POLLING_LOG	
				APS_LOG("%s:ps raw 0x%x -> value 0x%x \n",__FUNCTION__, stk3x1x_obj->ps, dat);			
#endif			
			
			
			if(stk3x1x_obj->ps_distance_last == 0){
				if((stk3x1x_obj->ps - stk3x1x_obj->ps_min) > 600){
					stk3x1x_obj->ps_thd_update = true;
					atomic_set(&stk3x1x_obj->ps_threshold_high, stk3x1x_obj->psi + 160); 
					atomic_set(&stk3x1x_obj->ps_threshold_low, stk3x1x_obj->psi + 150); 
					if((err = stk3x1x_write_ps_high_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_threshold_high))))
					{
						APS_ERR("write high thd error: %d\n", err);      
					}
					
					if((err = stk3x1x_write_ps_low_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_threshold_low))))
					{
						APS_ERR("write low thd error: %d\n", err);      
					}							

					APS_LOG("near update HT=%d, LT=%d\n", atomic_read(&stk3x1x_obj->ps_threshold_high), atomic_read(&stk3x1x_obj->ps_threshold_low));			

				}
			}else{				
				if(stk3x1x_obj->ps_thd_update){
					APS_LOG("far ps thd update ...\n");		
					stk3x1x_obj->ps_thd_update = false;
					err = stk3x1x_ps_val();
					if(err == 0){
						atomic_set(&stk3x1x_obj->ps_threshold_high, stk3x1x_obj->ps + stk3x1x_obj->stk_ht_n_ct); 
						atomic_set(&stk3x1x_obj->ps_threshold_low, stk3x1x_obj->ps + stk3x1x_obj->stk_lt_n_ct); 
...
//	obj->hw = get_cust_alsps_hw();
	...
	obj->first_boot = true;			 
	obj->als_correct_factor = 1000;
	atomic_set(&obj->ps_threshold_high, obj->hw->ps_threshold_high);//  obj->hw->ps_threshold_high
	...
	APS_LOG("%s:als_debounce=0x%x, state_val=0x%x, psctrl_val=0x%x, alsctrl_val=0x%x, ledctrl_val=0x%x, wait_val=0x%x, int_val=0x%x\n", 
		__FUNCTION__,atomic_read(&obj->als_debounce), atomic_read(&obj->state_val), atomic_read(&obj->psctrl_val), atomic_read(&obj->alsctrl_val), 
