...
int force_get_tbat(kal_bool update)
{
	...
	if(bat_temperature_val == (-20))//added by maxyu 170310 for NTC OPEN
	{
		bm_print(BM_LOG_CRTI, "[force_get_tbat][MaxYu] fixed TBAT=25 t, for NTC OPEN \n");
		return 25;
	}
...
void sw_oam_init_v2(void)
{
	...
	if (plugout_status == 0 && bat_is_charger_exist() == KAL_FALSE) {
		if (g_rtc_fg_soc == 0) {
			...
			//added by maxyu 170405 for boot
...
void oam_init(void)
{
	...
	if (gFG_voltage != g_booting_vbat) { //modified by maxyu 170517 for recoverymode
		if (vbat_capacity - gFG_capacity_by_v > 15){
		gFG_capacity_by_v_init = vbat_capacity;
		batt_meter_cust_data.cust_poweron_delta_capacity_tolrance = 70;
		}
		else
		gFG_capacity_by_v_init = gFG_capacity_by_v;
	}
...
#if defined(SOC_BY_SW_FG)
	battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &hw_ocv_after_sleep);
	if (_g_bat_sleep_total_time > 1800) {	/*1800 0.5hr,3600 1hr */
		//if (hw_ocv_after_sleep < g_hw_ocv_before_sleep) 
		if (fgauge_read_d_by_v(hw_ocv_after_sleep) > oam_d0)
		{
