...
void wake_up_bat(void)
{
	...
	fg_wake_up_bat = KAL_TRUE; //added by maxyu 170318 for charger
...
void check_battery_exist(void)
{
#if 1///defined(CONFIG_DIS_CHECK_BATTERY)
	...
	if (baton_count >= 3) {
		if ((g_platform_boot_mode == META_BOOT) || (g_platform_boot_mode == ADVMETA_BOOT)
			...
			battery_log(BAT_LOG_CRTI, "[BATTERY] [MaxYu]Battery is not exist, power off FAN5405 and system (%d)\n",baton_count);

				//marked by maxyu 170310 for NTC OPEN
			//battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
