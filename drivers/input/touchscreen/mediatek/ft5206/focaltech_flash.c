...
*******************************************************************************/
static u8 g_uc_tp_fm_tpid = 0;
static int g_uc_tp_fm_len = 0;
/*******************************************************************************
* Static variables
*******************************************************************************/
static unsigned char CTPM_FW[54*1024] = {};

static unsigned char CTPM_FW_ZHIHENG[] = {
	#include "ZHGF-1486_0x23_Ver0x19_20170104_app.i"
};

static unsigned char CTPM_FW_DEBIDA[] = {
	#include "DBD_677A1_0x25_Ver0x20_20170104_app.i"
};

static unsigned char CTPM_FW_DAWOSI[] = {
	#include "FT6336U_A1107A0_720X1280_Ver0x02_20170712_app.i"
};
...
{
	int ui_sz = g_uc_tp_fm_len;
	
#if 0	
	{
		int i = 0;
		for(;i < g_uc_tp_fm_len;i ++)
		{
			printk("0x%x  ",CTPM_FW[i]);
			if(i % 16 == 0)
			{
				printk("\n");
			}
	...
	*/
		...
		||(fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x06) ||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
	{
			...
			&& (CTPM_FW[fw_len - 3] ^ CTPM_FW[fw_len - 4]) == 0xFF) 
		{
			...
			if ((fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x08) ||(fts_updateinfo_curr.CHIP_ID==0x0a))
			{
				i_ret = fts_5x06_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
			}
			else if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) ||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14))
			{
				i_ret = fts_5x36_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
			}
			else if ((fts_updateinfo_curr.CHIP_ID==0x06))
			{
				i_ret = fts_6x06_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
			}
			if (i_ret != 0)
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x36))
	{
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x64))
	{
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6336GU_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
		if (i_ret != 0)
		{
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x58))
	{
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5822_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
		if (i_ret != 0) 
		{
			dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
		} 
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x59))
	{
	    	if (fw_len < 8 || fw_len > 54*1024) 
		{
    		...
    		pbt_buf = CTPM_FW;
	   	 /*call the upgrade function*/
    		i_ret = fts_5x26_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
    		if (i_ret != 0) 
		{
    			dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
    		} 
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x86))
	{
		...
		i_ret =  fts_8606_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x87))
	{
		...
		i_ret =  fts_8716_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x0E))
	{
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_3x07_ctpm_fw_upgrade(client, pbt_buf, g_uc_tp_fm_len);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	return i_ret;
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	auto_update = fts_read_reg(client, FTS_REG_VENDOR_ID,&g_uc_tp_fm_tpid);
	if(auto_update < 0)
	{
		printk("lipy error error error\n");
		return -EIO;
	}
	
	switch(g_uc_tp_fm_tpid)
	{
	case 0x23:	
		g_uc_tp_fm_len = sizeof(CTPM_FW_ZHIHENG);
		memcpy(CTPM_FW,CTPM_FW_ZHIHENG,g_uc_tp_fm_len);
		break;
	
	case 0x25:	
		g_uc_tp_fm_len = sizeof(CTPM_FW_DEBIDA);
		memcpy(CTPM_FW,CTPM_FW_DEBIDA,g_uc_tp_fm_len);	
		break;
		
	case 0x12:	
		g_uc_tp_fm_len = sizeof(CTPM_FW_DAWOSI);
		memcpy(CTPM_FW,CTPM_FW_DAWOSI,g_uc_tp_fm_len);	
		break;
	...
	printk("lipy %s uc_host_fm_ver = 0x%x,uc_tp_fm_ver = 0x%x,g_uc_tp_fm_tpid = 0x%x\n", __func__, uc_host_fm_ver,uc_tp_fm_ver,g_uc_tp_fm_tpid);
	if (uc_tp_fm_ver == FTS_REG_FW_VER || uc_tp_fm_ver < uc_host_fm_ver) 
	{
		printk("lipy FTS_REG_FW_VER = 0x%x \n",FTS_REG_FW_VER);
		msleep(100);
