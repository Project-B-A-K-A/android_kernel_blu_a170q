...
static int tpd_enable_ps(int enable)
{
	...
	if (enable)
	{
		...
		printk("[proxi_fts]ps function is on\n");	
	}
	else
	{
		state &= 0xfe;//0x00;	
		tpd_proximity_flag = 0;
		printk("[proxi_fts]ps function is off\n");
	}
	...
	printk("[proxi_fts]write: 0xB0's value is 0x%02X\n", state);
	return 0;
}
 /************************************************************************
* Name: tpd_ps_operate
...
static ssize_t show_eint_dump_status(struct device *dev, struct device_attribute *attr, char *buf)
{
    int len = 0;
	unsigned eint = 5;//touch_irq;
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "[EINT] eint:%d,mask:%x,pol:%x,deb:%x,sens:%x\n",eint,
    mt_eint_get_mask(eint), mt_eint_get_polarity(eint),
    mt_eint_get_debounce_cnt(eint), mt_eint_get_sens(eint));
...
static ssize_t store_eint_dump_status(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned eint = 5;
	if (!strcmp(buf, "unmask"))
		mt_eint_unmask(eint);
...
#endif
static DEVICE_ATTR(tpd_eint_status, 0664, show_eint_dump_status, store_eint_dump_status);

static struct device_attribute *ft5x0x_attrs[] = {
#ifdef CONFIG_MTK_SENSOR_HUB_SUPPORT
	&dev_attr_tpd_scp_ctrl,
#endif
    &dev_attr_tpd_eint_status,
};
	...
	do {
		...
		set_current_state(TASK_RUNNING);
			 ...
			 if (tpd_proximity_flag == 1)
			 {
				...
				//i2c_smbus_read_i2c_block_data(fts_i2c_client, 0xB0, 1, &state_proximity);

				ret_proximity = fts_read_reg(fts_i2c_client, 0xB0,&state_proximity);
				if (ret_proximity<0) 
				{
					printk("[Focal][Touch] read value fail");
					//return ret_proximity;
				}
	           		printk("proxi_fts 0xB0 state_proximity value is 1131 0x%02X\n", state_proximity);
				if(!(state_proximity&0x01))
				{
					tpd_enable_ps(1);
				}
				//i2c_smbus_read_i2c_block_data(fts_i2c_client, 0x01, 1, &proximity_status);
				ret_proximity = fts_read_reg(fts_i2c_client, 0x01,&proximity_status);
				if (ret_proximity<0) 
				{
					printk("[Focal][Touch] read value fail");
					//return ret_proximity;
				}
	            		printk("proxi_fts 0x01 value is 1139 0x%02X\n", proximity_status);
				...
				printk("tpd_proximity_detect 1149 = %d\n", tpd_proximity_detect);
				if ((err = tpd_read_ps()))
				{
					printk("proxi_fts read ps data 1156: %d\n", err);	
				}
				sensor_data.values[0] = tpd_get_ps_value();
				sensor_data.value_divide = 1;
				sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
				...
				//{
				//	printk(" proxi_5206 call hwmsen_get_interrupt_data failed= %d\n", err);	
				//}
			}  
	...
	if (retval != 0)
		...
		if (tpd_proximity_flag == 1)
		{
			if(tpd_proximity_flag_one == 1)
			{
				...
				if(mt_eint_get_mask(eint))
					enable_irq(touch_irq);
					
				printk(TPD_DEVICE " tpd_proximity_flag_one \n"); 
				return;
			}
		}
	#endif	
...
#else
	spin_lock_irqsave(&fts_irq_lock, flags); //add
	enable_irq(touch_irq);
	spin_unlock_irqrestore(&fts_irq_lock, flags); //add
    dump_stack();

	if(mt_eint_get_mask(eint))
		enable_irq(touch_irq);

	printk("[EINT][tpd_resume][enable_irq] eint:%d,mask:%x,pol:%x,deb:%x,sens:%x\n", eint,
    mt_eint_get_mask(eint), mt_eint_get_polarity(eint),
    mt_eint_get_debounce_cnt(eint), mt_eint_get_sens(eint));

	printk("tpd_resume exit \n");	
#endif
...
static void tpd_suspend(struct device *h)
{
	...
	if (tpd_proximity_flag == 1)
	{
		...
		printk("Ttpd_proximity_flag_one,return \n");
		return;
	}
	#endif
	...
	printk("[EINT][tpd_suspend][disable_irq] eint:%d,mask:%x,pol:%x,deb:%x,sens:%x\n", eint,
    mt_eint_get_mask(eint), mt_eint_get_polarity(eint),
    mt_eint_get_debounce_cnt(eint), mt_eint_get_sens(eint));
